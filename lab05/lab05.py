...
class LinkedList:
    ...
    def cursor_insert(self, value):
        ...
        new node"""
        x = LinkedList.Node(value, self.cursor, self.cursor.next)
        self.cursor.next = x
        self.cursor.next.prior = x
        self.cursor = x
        self.length += 1
    ...
    def index(self, value, i=0, j=None):
        ...
        if j != None: j = self._normalize_idx(j)
        counter = 0
        cur = self.head.next
        for a in range(self.length):
            if (j == None or counter < j) and counter >= i and cur.val == value:
                return counter
            cur = cur.next
            counter +=1
        raise ValueError
    ...
    def extend(self, other):
        ...
        ### BEGIN SOLUTION
        for i in other:
            self.append(LinkedList.Node(i).val)
        ### END SOLUTION
